...
const HEADERS = {
  'User-Agent': 'Mozilla/5.0 (compatible; SAS-Sports/2.3; Cloudflare-Worker)',
...
function makeEvent({school,sport,status,relation,opponent,date,time,schoolScore,oppScore,sourceUrl,now}){
  const start=parseDate(date,time); let effective=status;
  if(status==='Upcoming' && start){ const a=new Date(start), b=now; if(a.getUTCFullYear()===b.getUTCFullYear()&&a.getUTCMonth()===b.getUTCMonth()&&a.getUTCDate()===b.getUTCDate()) effective='Today'; }
  ...
  return {
    ...
    headline:schoolScore&&oppScore?`${schoolScore}–${oppScore}`:null, team_summaries:[], results:[],
    source:{name:'Official athletics live schedule',url:sourceUrl,updated_at:now.toISOString()}, result_count:0,
...
async function fetchLive(schoolId,sport){
  ...
  for(const url of candidateUrls(school,sport)){
    try{
      ...
      const html=await r.text(); const events=parseHtml(html,school,sport,r.url||url,now);
    ...
    }catch(e){errors.push(`${url}: ${e?.name||'FetchError'}`);}
  }
...
export default {
  async fetch(request, env){
    ...
    if(url.pathname==='/api/status') return json({name:'SAS Sports API',version:'2.3.0',mode:'cloudflare-worker-live',web_live_mode:true,school_catalog_count:schools.length,web_path:'/'});
    ...
    if(url.pathname==='/live/status'){
      const school=url.searchParams.get('school'), sport=url.searchParams.get('sport');
      if(!school||!sport) return json({detail:'school and sport are required'},400);
      const result=await fetchLive(school,sport); return json({school,sport,live_source_used:result.live_source_used,source_url:result.source_url,fetched_at:result.fetched_at,event_count:result.events.length,error:result.error});
    }
