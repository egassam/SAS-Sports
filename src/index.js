...
function parseDate(dateText,timeText){if(!dateText)return null;const raw=`${dateText}${timeText?` ${timeText.replace(/\./g,'')}`:''}`.trim(),d=new Date(raw);return Number.isNaN(d.getTime())?null:d.toISOString();}
